...
export function QuickViewDialog({
  ...
  return (
    ...
    >
      <DialogContent className="max-w-3xl w-[calc(100vw-2rem)] p-0 overflow-hidden bg-background border border-border text-foreground">
        <div className="grid md:grid-cols-2">
          {/* Image */}
          <div className="relative aspect-square md:aspect-auto md:min-h-[480px] bg-gradient-to-br from-card to-background overflow-hidden">
            <div
            ...
            />
            <div className="absolute top-4 left-4 rounded-full bg-background/70 backdrop-blur px-3 py-1 text-[10px] font-bold uppercase tracking-widest">
              {product.tag}
            </div>
          </div>

          {/* Details */}
          <div className="p-6 sm:p-8 flex flex-col gap-5 overflow-y-auto max-h-[80vh]">
            <div>
            ...
            <div className="mt-auto flex flex-col sm:flex-row gap-3 pt-2">
              <button
                onClick={handleBuy}
                disabled={!size || added}
                className="flex-1 inline-flex items-center justify-center gap-2 rounded-full bg-primary text-primary-foreground px-6 py-4 text-xs font-bold uppercase tracking-widest disabled:opacity-50 disabled:cursor-not-allowed hover:scale-[1.02] transition"
              >
                {added ? (
                  <>
              ...
              <button
                ...
                className="rounded-full border border-border bg-card/40 px-6 py-4 text-xs font-bold uppercase tracking-widest hover:bg-card"
              >
                Keep Browsing
              </button>
            </div>
